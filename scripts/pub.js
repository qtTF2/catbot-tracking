...
},{"add-zero":1,"parse-ms":5}],4:[function(require,module,exports){
/*!
...
function updateIPCData(row, id, data) {
	...
	if (!data.heartbeat || time < 4) {
		row.find('.client-status').removeClass('error warning').text('Client OK ' + time);
	} else if (time < 45) {
		row.find('.client-status').removeClass('error').addClass('warning').text('Client Frozen Warning ' + time);
	} else {
		row.find('.client-status').removeClass('warning').addClass('error').text('Cilent Likely dead ' + time);
	}
...
function updateUserData(bot, data) {
	...
	if (data.state != 5) {
		row.find('.active').text('Bot offline');
	}
...
function addClientRow(botid) {
    ...
    row.append($('<td></td>').attr('class', 'client-name').text(' '));
    ...
    row.append($('<td></td>').attr('class', 'client-steam').text(' '));
    row.append($('<td></td>').attr('class', 'client-status active').text('None'));
    //row.append($('<td></td>').attr('class', 'client-uptime-queue active').text('0:00'));
    row.append($('<td></td>').attr('class', 'client-total active').text('0'));
    row.append($('<td></td>').attr('class', 'client-score connected active').text('0'));
    row.append($('<td></td>').attr('class', 'client-uptime-server connected active').text('0:00'));
    row.append($('<td></td>').attr('class', 'client-team connected active').text('None'));
    row.append($('<td></td>').attr('class', 'client-class connected active').text('None'));
    row.append($('<td></td>').attr('class', 'client-health connected active').text('0/0'));
    row.append($('<td></td>').attr('class', 'client-ip connected active').text('0.0.0.0'));
    row.append($('<td></td>').attr('class', 'client-map connected active').text('None'));
    row.append($('<td></td>').attr('class', 'client-players connected active').text('0'));
    $('#clients').append(row);
    return row;
}
...
function refreshComplete() {
	...
	request.get({
		...
		var b = JSON.parse(b);
		console.log(b);
		console.log(b.bots.b0)
		for (var i in b.bots && b.state == 5) {
			bot_online++;
		}
		for (var i in b.bots) {
			count++;
		...
		last_count = count;
		let bot_total = document.getElementById("counter");
		bot_total.textContent = count;
		
		let online = document.getElementById("online");
		online.textContent = bot_online;
